...
def save_to_robot_file(session_id, history):
    """Save conversation to a .robot file."""
    ...
    try:
        with open(robot_file, 'w') as file:
            ...
            for entry in history.get(session_id, []):
                prompt = entry['prompt']
                response = entry['response']
                test_case_name = prompt[:30].replace(" ", "_").replace("\n", "_")
                file.write(f"{test_case_name}\n")
                file.write(f"    [Documentation]  {prompt}\n")
                file.write(f"    Log  {response}\n\n")

        print(f"Conversation saved to {robot_file}")
    except Exception as e:
...
def prompt_chaining(ai, conversation_history, session_id):
    ...
    # Save the history to a .robot file
    save_to_robot_file(session_id, history)
